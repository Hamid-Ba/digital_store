...
class Payment(models.Model):
    ...
    desc = models.CharField(
        max_length=125, null=True, blank=True, verbose_name="توضیحات"
    )
    ref_id = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="شناسه پرداخت"
    )
    authority = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="کد پرداخت"
    )
    is_payed = models.BooleanField(default=False, verbose_name="آیا پرداخت شده؟")
    payed_date = models.DateTimeField(
        null=True, blank=True, verbose_name="تاریخ پرداخت"
    )
    created_date = models.DateTimeField(auto_now_add=True, verbose_name="تاریخ ایجاد")
    status = models.PositiveSmallIntegerField(
        choices=PaymentStatus.choices,
        default=PaymentStatus.PAYMENT_CREATED,
        verbose_name="وضعیت",
    )
